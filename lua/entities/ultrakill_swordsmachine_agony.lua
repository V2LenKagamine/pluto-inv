...
function ENT:OnSpawn()
  ...
  UltrakillBase.AddBoss( self, "#ultrakill.swordsmachine.agony.boss", 1  )

  self:SetCooldown( "Attack", 0.5 ) -- Attack Delay of 0.5s

  for K, V in ipairs( self:GetAllies() ) do

    if V:GetClass() ~= "ultrakill_swordsmachine_tundra" or IsValid( V.Symbiote ) then continue end
...
function ENT:OnDowned()
  ...
  self:PlaySequenceAndMove( "Knockdown", 1, function( self, Cycle )
    ...
    fHealth = MApproach( fHealth, fTarget, fChange * 1.1 )
