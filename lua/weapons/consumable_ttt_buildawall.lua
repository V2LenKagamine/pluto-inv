...
SWEP.Description = "A small device for summoning crude 200 health shields that can be walked, but not shot, through.\nCan change rotation with 'Reload'; and lock position with 'Aim'.\n2 Uses."
...
SWEP.Ortho = {-1, 1.5, angle = Angle(0, 90, -80)}
...
function SWEP:Think()
    if (CLIENT) then
        if(!IsValid(self.c_Model)) then
		    ...
		    self.c_Model:SetModel("models/pandora/barricade.mdl")
...
function SWEP:PrimaryAttack()
    ...
    local ent = ents.Create("pandora_barricade")
