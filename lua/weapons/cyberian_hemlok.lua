...
SWEP.Primary.BurstInbetweenDelay = 0.065 -- The delay that's inbetween each shot of a burst.
SWEP.Primary.BurstDelay = 0.25 -- The delay between each burst.
...
SWEP.IronSightsPos = Vector(0, 0.201, 1.406)
...
if ( self:Clip1() == self.Primary.ClipSize or self.Owner:GetAmmoCount( self.Primary.Ammo ) <= 0 ) then return end
    ...
    self:SetIronsights( false )
    self:SetZoom( false )
    self.Weapon:EmitSound("wpn_hemlok_reload_removemag_fr15_2ch_v1_01.ogg")
end
...
function SWEP:Think()
	...
	if self:GetReloadEndTime() ~= 0.0 then
		if self:GetReloadEndTime() <= CurTime() then
			self:SetReloadEndTime(0.0)
            self.Weapon:EmitSound("wpn_hemlok_reload_empty_chargeforward_fr76_2ch_v1_01.ogg")
	...
	if self:GetBurstShotEndTime() <= CurTime() then
		...
		elseif self:CanPrimaryAttack() then -- We still have shots to fire.
			self:FireShot()
            self.Weapon:EmitSound("fire_" .. math.random(1, 7) .. ".ogg") -- Cursed.
