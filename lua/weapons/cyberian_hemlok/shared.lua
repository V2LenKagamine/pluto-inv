...
if ( self:Clip1() == self.Primary.ClipSize or self.Owner:GetAmmoCount( self.Primary.Ammo ) <= 0 ) then return end
    ...
    self:SetIronsights( false )
    self:SetZoom( false )
    self.Weapon:EmitSound("weapons/hemlok/wpn_hemlok_reload_removemag_fr15_2ch_v1_01.ogg")
end
...
function SWEP:Think()
	...
	if self:GetReloadEndTime() ~= 0.0 then
		if self:GetReloadEndTime() <= CurTime() then
			self:SetReloadEndTime(0.0)
            self.Weapon:EmitSound("weapons/hemlok/wpn_hemlok_reload_empty_chargeforward_fr76_2ch_v1_01.ogg")
